...
const roleConfig = {
    ...
    supervisor: { label: '🛡️ Supervisor', className: 'bg-purple-50 text-purple-800 border-purple-200' },
    team_member: { label: '👤 Team Member', className: 'bg-green-50 text-green-800 border-green-200' },
};
            ...
            <DialogContent className="sm:max-w-lg p-0 overflow-hidden rounded-2xl gap-0">
                ...
                <div className="flex items-center gap-4 px-6 py-4 border-b">
                    ...
                    <div>
                        ...
                        <Badge variant="outline" className={`text-xs rounded-full ${role.className}`}>
                            {role.label}
                        </Badge>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-2">
                    {[
                        ...
                        { label: 'Status', value: selectedMember.isDeleted ? 'Inactive' : 'Active', color: selectedMember.isDeleted ? 'text-red-500' : 'text-green-600' },
                        { label: 'Joined', value: format(new Date(selectedMember.createdAt), 'dd MMM, yyyy') },
                    ].map((item, i) => (
                        <div key={i} className={`px-6 py-3 border-b ${i % 2 === 0 ? 'border-r' : ''}`}>
                            <p className="text-sm text-gray-500 uppercase tracking-wide mb-1">{item.label}</p>
                            <p className={`text-base font-medium capitalize ${item.color ?? ''}`}>{item.value}</p>
                        </div>
                    ))}
                </div>
                ...
                <div className="flex justify-between gap-2 mt-8 px-5 pb-5">
                    ...
                    <Button
                        ...
                        onClick={() => {
                            onOpenChange(false);
                            router.push(`/vendor/team-members/update-member/${selectedMember._id}`);
                        }}
