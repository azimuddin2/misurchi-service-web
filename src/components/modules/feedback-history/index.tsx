            ...
            <div className=" lg:p-5 min-h-fit">
                ...
                <div className="flex flex-col lg:justify-between lg:flex-row gap-4 lg:mt-0 mt-5 mb-5">
                    ...
                    <input
                        ...
                        onChange={(e) =>
                            handleDateSelect(e.target.value ? new Date(e.target.value) : undefined)
                        }
                ...
                {tickets.length === 0 ? (
                    <div className="text-center py-10 text-gray-400 text-sm">
                        ...
                        />
                        <span className="font-medium mt-2 text-gray-600 text-base capitalize">No feedback found.</span>
                    </div>
                ) : (
                    <div className="divide-y divide-gray-100">
                        {tickets.map((ticket) => (
                            <div key={ticket._id} className="py-4">
                                ...
                                <p className="text-sm text-gray-600 mb-1">Feedback: {ticket.message}</p>
                                ...
                                <p className={`text-sm font-semibold mb-2 ${statusColor[ticket.status] ?? 'text-gray-500'}`}>
                                    Status: {ticket.status}
                                </p>
                                <p className="text-sm text-yellow-500 mb-1">Was this response helpful?</p>
                                <div className="flex items-center gap-2">
                                    <button
                                        onClick={() => handleHelpful(ticket._id!, true)}
                                        className={`flex items-center gap-1 text-sm px-3 py-1 rounded border transition cursor-pointer ${ticket.isHelpful === true
                                    ...
                                    <button
                                        ...
                                        className={`flex items-center gap-1 text-sm px-3 py-1 rounded border transition cursor-pointer ${ticket.isHelpful === false
